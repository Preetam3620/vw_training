...
//		log.info("As written, this test will always pass!");
	...
	void dummyTest1() {
		...
		assertAll("Assert All of these", () -> assertEquals(expected, actual, "INCONCEIVABLE!"),
				...
				() -> assertNotNull("A String", "INCONCEIVABLE!"), () -> assertTrue(nullValue == null));

		/*
		 * The cool thing about assertAll() is that all of the assertions contained
		 * within it are performed, even if one or more of them fail. Contrast this to
		 * the code in previous test, where if any assertion fails, the test fails at
		 * that point, meaning no other assertions will be performed.
		 */
	}
	...
	void afterEach() {
		...
		// cleanup
		// file close, database connection
	}
